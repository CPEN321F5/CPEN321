...
var item_id
beforeAll(async () => {
    var response = await request.post("/item/postitem/").send({name: 'Desk' , Seller : "1", Price : "10"}).set('Accept', 'application/json')
...
test("Updating Item - update exsting item", async () => {
    var response = await request.put("/item/updateitem/").send({ItemID: item_id , Price : "500"}).set('Accept', 'application/json')
...
test("Updating Item - update non existing item", async () => {
    ...
    var response = await request.put("/item/updateitem/").send({ItemID: "not a id" , Price : "500"}).set('Accept', 'application/json')
    expect(response.status).toBe(404)
})

test("Updating Item - update no item id", async () => {
    //user now exist as the first test would have created the use
    var response = await request.put("/item/updateitem/").send({Price : "500"}).set('Accept', 'application/json')
